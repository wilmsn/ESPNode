...
void Sensor_Bosch::mqtt_stat(String& _mqtt_stat) {
  _mqtt_stat += String("\"") + keyword + String("\":\"") + String(bmx_sensor.getTemperature(),1) +
                String("\",\"") + keyword2 + String("\":\"") + String(bmx_sensor.getPressure(),0) + String("\"");
  if ( bmx_sensor.hasHumidity() ) {
    _mqtt_stat += String(",\"") + keyword3 + String("\":\"") + String(bmx_sensor.getHumidity(),0) + String("\"");
  }
}
