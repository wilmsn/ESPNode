...
*/
  Switch_OnOff::begin(html_place, label, mqtt_name, keyword, true, true, 20, 100, 1, "volume", "Lautstärke");
...
void RotaryTest::loop(time_t mytime) {
  rotarymodul.loop();
  switch (rotarymodul.changed()) {
  case 1:
    if ( rotarymodul.curLevel() == 0 ) set_slider(rotarymodul.curValue());
    if ( ! obj_value && (rotarymodul.curValue() > 0) ) set_switch(1);
    if ( obj_value && (rotarymodul.curValue() == 0) ) set_switch(0);
//    Serial.printf("Rotary Level %u Position: %u Slider: %u\n", rotarymodul.curLevel(), rotarymodul.curVal(), get_slider_val());
    break;
  case 2:
    Serial.printf("Rotary Level %u Position: %u Slider: %u\n", rotarymodul.curLevel(), rotarymodul.curValue(), get_slider_val());
  break;
  case 3: 
